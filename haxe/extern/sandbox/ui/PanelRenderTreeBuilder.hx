...
extern class PanelRenderTreeBuilder {
	overload function AddAttribute<T>(sequence:Int, value:system.Action1<T>):Void;

	overload inline function AddAttribute<T>(sequence:Int, value:T->Void):Void {
		untyped __cs__("{0}.AddAttribute({1}, v => {2}.__hx_invoke1_o(v))", this, sequence, value);
	}
	...
	overload inline function AddAttributeAction(sequence:Int, attrName:String, value:Void->Void):Void {
		untyped __cs__("{0}.AddAttributeAction({1}, {2}, () => {3}.__hx_invoke0_o())", this, sequence, attrName, value);
	}
	...
	overload function AddReferenceCapture<T>(sequence:Int, current:T, value:system.Action1<T>):Void;

	overload inline function AddReferenceCapture<T>(sequence:Int, current:T, value:T->Void):Void {
		untyped __cs__("{0}.AddReferenceCapture({1}, {2}, p => {3}.__hx_invoke1_o(default(double), p))", this, sequence, current, value);
	}
	...
	function SetRenderFragment<T>(setter:system.Action2<T, microsoft.aspnetcore.components.RenderFragment>,
		builder:microsoft.aspnetcore.components.RenderFragment):Void;
	function SetRenderFragmentWithContext<T, U>(getter:system.Func2<T, microsoft.aspnetcore.components.RenderFragment1<U>>,
		setter:system.Action2<T, microsoft.aspnetcore.components.RenderFragment1<U>>, builder:microsoft.aspnetcore.components.RenderFragment1<U>):Void;
}
